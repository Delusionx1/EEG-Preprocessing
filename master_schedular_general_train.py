...
for i in range(process_num):
    p = subprocess.Popen(["python", "main_test.py", "-o", \
        '', '-p', str(i+1), '-g', str(0), '-m', 'eegNet', '-d', 'Koera_54'])    
    all_p.append(p)
    current_participant_num += 1
    ...
    for i in range(len(all_p)):
        ...
        if(all_return_code[i] == 0):
            if(current_participant_num <= participant_amount):
                all_p[i] = subprocess.Popen(["python", "main_test.py", "-o", '', '-p', \
                    str(current_participant_num), '-g', str(0), '-m', 'eegNet', '-d', 'Koera_54']) 
                current_participant_num += 1
