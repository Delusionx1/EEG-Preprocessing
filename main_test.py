...
from dataset_pick_and_loading import load_BCIIV2a_data_epoch, generate_balanced_index, slice_the_time,\
    ...
    load_fatigue_detection_by_participoant_path
...
from SE_FBCNet_Ablation import SE_FBCNet_Ablation
...
if( model_name == 'FBCSP' or model_name == 'Riemannian'):
    ...
    training_data, training_labels = slice_the_time(data_all, all_labels, training_data_index, time_window_slice,step_len)
    validation_data, validation_labels = slice_the_time(data_all, all_labels, validation_data_index, time_window_slice,step_len)
    testing_data, testing_labels = slice_the_time(data_all, all_labels, testing_data_index, time_window_slice,step_len)
    ...
    if(model_name == 'LGG' or model_name == 'SE_FBCNet' or model_name == 'SE_FBCNet_spatial'or model_name == 'SE_FBCNet_Ablation'):
        ...
        if(model_name =='SE_FBCNet_Ablation'):
                model = SE_FBCNet_Ablation(nChan = num_channels, nTime = time_window_slice, nClass = num_of_classes, idx = idx, idx_local_graph = idx_local_graph, chann_locs = my_loc_channels).to(device)
    ...
    path_training_results = data_set_id+'//'+model_name + str(training_rate)
    if(not os.path.exists(path_training_results)):
        os.makedirs(path_training_results)
    ...
    for i, model_t in enumerate(model.multi_conv):
        for name, param in model_t.named_parameters():
            if(i == 0):
                final_param = param
            else:
                final_param = torch.cat((final_param, param), dim=2)
            break
    param_data_np = final_param.cpu().detach().numpy()
    param_data_np = np.squeeze(param_data_np, axis=1)
    param_data_np = np.squeeze(param_data_np, axis=2).T
    param_data_np_mean = np.mean(np.abs(param_data_np),axis=1).reshape(param_data_np.shape[0],1)
    ...
    sns.heatmap(param_data_np, cbar=True)
    plt.yticks(np.arange(len(chans)), chans)
    plt.savefig('heatmap1.png')
    plt.show()
    plt.clf()

    sns.heatmap(param_data_np_mean, cbar=True)
    plt.yticks(np.arange(len(chans)), chans)
    plt.savefig('heatmap1_mean.png')
    plt.show()
    plt.clf()
    ...
    for epoch in range(epochs):
        ...
        for i,(inputs, targets) in enumerate(train_dl):
            if(model_name == 'FBCNet' or model_name == 'LGG' or model_name == 'tempoalNet' or model_name == 'tempoalNet_' or model_name == 'tempoalNet_me'or model_name == 'TSceptionIJCNN' or model_name == 'FBSCNet' or model_name == 'Bio_FSTNet' or model_name == 'SE_FBCNet' or model_name == 'SE_FBCNet_spatial'or model_name == 'SE_FBCNet_Ablation'):
            ...
            acc = round(accuracy_score(targets.tolist(), preds.data.tolist()),4)
        ...
        if((acc > highest_acc) and (epoch >= int(epochs*0.5))):
            ...
            print('Weights refreshed!')
    torch.save(model.state_dict(), last_results_dir)
    ...
    for i, model_t in enumerate(model.multi_conv):
        ...
        for name, param in model_t.named_parameters():
            ...
            if(i == 0):
                ...
                final_param_2 = torch.cat((final_param_2, param), dim=2)
            break

    param_data_np_2 = final_param_2.cpu().detach().numpy()
    param_data_np_2 = np.squeeze(param_data_np_2, axis=1)
    param_data_np_2 = np.squeeze(param_data_np_2, axis=2).T
    param_data_np_mean_2 = np.mean(np.abs(param_data_np_2),axis=1).reshape(param_data_np_2.shape[0],1)

    sns.heatmap(param_data_np_2, cbar=True)
    plt.yticks(np.arange(len(chans)), chans)
    plt.savefig('heatmap2.png')
    plt.show()
    plt.clf()

    sns.heatmap(param_data_np_mean_2, cbar=True)
    plt.yticks(np.arange(len(chans)), chans)
    plt.savefig('heatmap2_mean.png')
    plt.show()
    plt.clf()
    ...
    f.write('The pre-processing order is: '+preprocessing_order+'\n')
    f.write('Best model testing result and confuxion matrix is:\n'+str(acc)+'\n'+str(cm)+'\n')
    print('Best model testing result and confuxion matrix is:\n'+str(acc)+'\n'+str(cm)+'\n')

    predicted_last = []
    actual_last = []
    all_features_last = []
    model.load_state_dict(torch.load(last_results_dir, map_location=device))
    model = model.to(device)
    with torch.no_grad():
        for i,(inputs, targets) in enumerate(test_dl):
            if(model_name == 'FBCNet' or model_name == 'LGG' or model_name == 'tempoalNet' or model_name == 'tempoalNet_' or model_name == 'tempoalNet_me'or model_name == 'TSceptionIJCNN' or model_name == 'FBSCNet' or model_name == 'Bio_FSTNet' or model_name == 'SE_FBCNet' or model_name == 'SE_FBCNet_spatial'or model_name == 'SE_FBCNet_Ablation'):
                inputs = transform(inputs.numpy())
            actual_last.extend(targets.tolist())
            targets = torch.squeeze(targets.type(torch.int64))
            inputs = inputs.to(device)
            targets = targets.to(device)
            optimizer.zero_grad()
            preds = model(inputs)
            all_features_last.extend(preds.data.tolist())
            loss += criterion(preds, targets)
            _, preds = torch.max(preds, 1)
            predicted_last.extend(preds.data.tolist())

    acc_last = round(accuracy_score(actual_last, predicted_last),4)
    if classes is not None:
        cm_last = confusion_matrix(actual_last, predicted_last, labels= classes)
    else:
        cm_last = confusion_matrix(actual_last, predicted_last)

    all_features_last = np.array(all_features_last)
    actual_last = np.array(actual_last).reshape(all_features_last.shape[0])
    print(all_features_last.shape, actual_last.shape)
    umap_results_dir = os.path.join(path_training_results_data, "Pre_processing_order_"+str(preprocessing_order)+"last.npz")
    np.savez(umap_results_dir, all_features_last, actual_last)
    npzfile = np.load(umap_results_dir)
    print('Load results are:', npzfile['arr_0'].shape, npzfile['arr_1'].shape)
    f.write('The pre-processing order is: '+preprocessing_order+'\n')
    f.write('Last model testing result and confuxion matrix is:\n'+str(acc_last)+'\n'+str(cm_last)+'\n')
    print('Last model testing result and confuxion matrix is:\n'+str(acc_last)+'\n'+str(cm_last)+'\n')
    f.close()